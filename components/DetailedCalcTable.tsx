...
import type { CotisationsDetail } from '@/lib/calcul';
...
const pct = (n: number) =>
  (n * 100).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 3 }) + ' %';
...
/** Reproduit la table "Cotisations et contributions sociales obligatoires" d'un bulletin Pajemploi réel. */
function CotisationsBulletin({ label, data }: { label: string; data: CotisationsDetail }) {
  const lignes = data.lignes.filter(l => l.montantSalarie > 0 || l.montantEmployeur > 0);
  if (lignes.length === 0) return null;

  return (
    <div className="overflow-x-auto rounded-[var(--radius)] border border-[var(--line)] bg-white">
      <div className="px-4 py-2.5 border-b border-[var(--line)] bg-[var(--paper)] text-[10px] font-semibold uppercase tracking-wider text-[var(--dust)]">
        Cotisations et contributions sociales obligatoires — {label}
      </div>
      <table className="w-full">
        <thead>
          <tr className="border-b border-[var(--line)]">
            <th className="text-left  px-4 py-2 text-[9px] font-semibold uppercase tracking-wider text-[var(--dust)]">Cotisation</th>
            <th className="text-right px-2 py-2 text-[9px] font-semibold uppercase tracking-wider text-[var(--dust)]">Base</th>
            <th className="text-right px-2 py-2 text-[9px] font-semibold uppercase tracking-wider text-[var(--dust)]">Taux sal.</th>
            <th className="text-right px-2 py-2 text-[9px] font-semibold uppercase tracking-wider text-[var(--dust)]">Mont. sal.</th>
            <th className="text-right px-2 py-2 text-[9px] font-semibold uppercase tracking-wider text-[var(--dust)]">Taux pat.</th>
            <th className="text-right px-4 py-2 text-[9px] font-semibold uppercase tracking-wider text-[var(--dust)]">Mont. pat.</th>
          </tr>
        </thead>
        <tbody>
          {lignes.map(l => (
            <tr key={l.label} className="border-b border-[var(--line)] last:border-b-0">
              <td className="px-4 py-1.5 text-xs text-[var(--ink)]">{l.label}</td>
              <td className="px-2 py-1.5 text-xs text-right tabular-nums text-[var(--dust)]">{eur(l.base)}</td>
              <td className="px-2 py-1.5 text-xs text-right tabular-nums text-[var(--dust)]">{l.tauxSalarie > 0 ? pct(l.tauxSalarie) : '—'}</td>
              <td className="px-2 py-1.5 text-xs text-right tabular-nums text-[var(--ink)]">{l.montantSalarie > 0 ? eur(l.montantSalarie) : '—'}</td>
              <td className="px-2 py-1.5 text-xs text-right tabular-nums text-[var(--dust)]">{l.tauxEmployeur > 0 ? pct(l.tauxEmployeur) : '—'}</td>
              <td className="px-4 py-1.5 text-xs text-right tabular-nums text-[var(--ink)]">{l.montantEmployeur > 0 ? eur(l.montantEmployeur) : '—'}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="bg-[var(--paper)] border-t border-[var(--line)]">
            <td className="px-4 py-2 text-xs font-semibold text-[var(--ink)]" colSpan={3}>Montant total des cotisations</td>
            <td className="px-2 py-2 text-xs font-semibold text-right tabular-nums text-[var(--ink)]">{eur(data.totalSalarie)}</td>
            <td />
            <td className="px-4 py-2 text-xs font-semibold text-right tabular-nums text-[var(--ink)]">{eur(data.totalEmployeur)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
...
export function DetailedCalcTable({ famA, famB, nounou, racOptionActive }: Props) {
  ...
  const coutTotalA = famA.brut + famA.cotisations.totalEmployeur + famA.transport + famA.entretien + famA.km;
  const coutTotalB = famB.brut + famB.cotisations.totalEmployeur + famB.transport + famB.entretien + famB.km;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <CotisationsBulletin label={famA.nom} data={famA.cotisations} />
      <CotisationsBulletin label={famB.nom} data={famB.cotisations} />
    </div>
  );
}
